...
class VendorMachineTest {
	...
	void shouldBuyProduct() {
		mvc.perform(
				post("/buy")
						...
						.content(objectMapper.writeValueAsString(
								new BuyProductRequest(exampleProduct.getId().toString(), 5))))
				.andDo(result -> {
					var response = objectMapper.readValue(result.getResponse().getContentAsString(),
							BuyProductResponse.class);
		...
		mvc.perform(
				...
				.andDo(
						result -> {
							var products = objectMapper.readValue(result.getResponse().getContentAsString(),
									new TypeReference<Collection<Product>>() {
									});
	...
	void shouldNoBuyProduct() {
		mvc.perform(
				post("/buy")
						...
						.contentType(MediaType.APPLICATION_JSON)
						.content(objectMapper.writeValueAsString(
								new BuyProductRequest(exampleProduct.getId().toString(), 50))))
				.andDo(result -> {
